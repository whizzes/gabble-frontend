<script lang="ts">
  ...
  let className = classNames(customClassNames, 'flex px-3 py-2 md:px-4 md:py-3 border-2 border-black rounded-lg font-medium placeholder:font-normal')
	...
	{#if label}
		<label for={name}  class="block font-medium text-gray-700 pb-1"
			>{label}</label
		>
	{/if}
